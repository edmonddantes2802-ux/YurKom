 ...
 * не расходится.
 *
 * ВНУТРИ ИНЛАЙН-СКРИПТА КОММЕНТАРИЕВ БЫТЬ НЕ ДОЛЖНО. Это шаблонная строка,
 * минификатор её не трогает — всё написанное там уезжает в HTML каждой
 * страницы. Пояснения к настройкам держим здесь.
 *
 * О `webvisor: true`. Записи сессий нужны для разбора юзабилити на старте.
 * Поля формы закрыты классом `ym-disable-keys` (`components/Qualifier.tsx`):
 * ситуация и телефон — персональные данные, их Вебвизор не пишет. Добавляя
 * новые поля ввода, вешать этот класс на них тоже.
 */

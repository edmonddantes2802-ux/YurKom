...
export const FALLBACK_REPLY_DAY = [
  'Передал ваше обращение юристу.',
  '',
  'Опишите пока подробнее: что произошло, когда, какие документы на руках. Так Иван сразу увидит картину целиком и ответит по существу.',
  '',
  `Срочно — звоните ${PHONE}`,
].join('\n');

export const FALLBACK_REPLY_DAY_REPEAT = [
  'Вижу ваше сообщение — оба уже у юриста.',
  '',
  `Если срочно, лучше позвонить: ${PHONE}`,
].join('\n');

export const FALLBACK_REPLY_NIGHT = [
  'Обращение записал. Сейчас ночь — юрист прочитает утром, после 9:00.',
  '',
  'Опишите пока ситуацию подробно: что произошло, когда, какие документы. Утром он увидит всё сразу.',
].join('\n');

export const FALLBACK_REPLY_NIGHT_REPEAT = [
  'Записал, всё сохранено. Юрист посмотрит утром, после 9:00.',
].join('\n');

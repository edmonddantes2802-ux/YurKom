...
```bash
npm test           # тесты, встроенный раннер Node (node:test), без зависимостей
npm run guard      # гейт: секреты в коде и в индексе git
```

Тесты лежат в `test/`, гоняют настоящие модули из `app/` и `lib/`. Типы снимает
сам Node, поэтому раннер не тянет ни транспайлер, ни тестовый фреймворк; резолв
алиаса `@/` и подпути `next/server` живёт в `scripts/module-hooks.mjs`.
...
| `TELEGRAM_WEBHOOK_SECRET` | только сервер, `/api/telegram/webhook` | вебхук отвечает 401 на всё, бот молчит (проверка fail-closed) |
